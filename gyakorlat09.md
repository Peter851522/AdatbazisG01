...
```js
MATCH(p:Person)-[:PRODUCED]->(m:Movie) WHERE p.name = 'Joel Silver' RETURN m.title
```
...
WHERE EXISTS ((p)-[:DIRECTED]->(m:Movie))
RETURN p.name
```
...
```js
MATCH (p:Person)-[:ACTED_IN]->(m:Movie)
WITH m.title AS cim, COUNT(p.name) AS szineszek
RETURN cim, szineszek
ORDER BY szineszek DESC, cim
LIMIT 10
```
...
```js
CREATE INDEX i_person_born FOR (p:Person) ON(p.name, p.born)
```
